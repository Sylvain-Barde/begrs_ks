...
class begrsSbc:
    ...
    def saveData(self, path):
        ...
        if not os.path.exists(path):
            ...
            saveDict = {'testSamples':self.testSamples,
                        ...
                        'posteriorESS':self.posteriorSamplesESS}
    ...
    def run(self, N, burn, init, numBins = 50, autoThin = True, essCutoff = 0):
        """
        Run the SBC analysis

        Produce a rank histogram with numBins bins by drawing (N-burn)
        samples for each parameter sample in the testing samples.

        Notes:
        - If auto-thin is active (true by default), the algorithm will
          draw posterior samples until the effective sample size of the draws
          is numBins. This setting allows to control for autocorrelation in
        ...
        - When auto-thin is active, the algorithm first draws (N - burn)
          ...
          calculation. This becomes:
              ...
              numBins/max(ESS, essCutoff) - 1

          By default this is set to 0, which is the same as specifying no
          cutoff.
        ...
        ----------
            ...
            init (ndarray):
                A centered vector of initial values for the parameter vector.
            numBins (int):
                Number of bins in histogram. The default is 50.
            autoThin (boolean):
        ...
        if numBins > N-burn-1:
            print(' - Warning, fewer bins than samples, adjusting N')
            N = burn + numBins - 1
        ...
        for ind in range(self.testData.shape[-1]):
            ...
            if self.hist is None:
                ...
                self.hist = np.zeros([int(numBins), self.numParam])
